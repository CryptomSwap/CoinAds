...
const RATE_LIMITS = {
  ...
  // Higher limit for tracking APIs: 300 requests per minute
  TRACK_API: {
    maxRequests: 300,
    windowMs: 60 * 1000, // 1 minute
  },
...
// This Map will be lost on server restarts and doesn't scale across multiple instances
...
function getClientIP(request: NextRequest): string {
  ...
  // Fallback to connection IP (may be proxy IP in production)
  return request.ip || 'unknown';
}

// Helper function to check if path matches tracking API
...
function checkRateLimit(
  ...
  const now = Date.now();
  const config = isTrackAPI ? RATE_LIMITS.TRACK_API : RATE_LIMITS.DEFAULT;
...
export function middleware(request: NextRequest) {
  ...
  // Check if this is a tracking API
